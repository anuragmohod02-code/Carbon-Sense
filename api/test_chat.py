...
prompts = [
    ...
    },
    {
        ...
        "say": "Second message. Without Carbon Sense — it re-sends the entire first exchange. That's already 480 tokens just to ask a 20-token question. Carbon Sense stored one memory chunk from exchange one — nine tokens — and injected only that. Watch the gap open."
    },
    {
        "text": "We're deploying this to Railway. How should I store the JWT secret key securely?",
        "say": "Third message. Naive system is now at 900 tokens per request — and growing. Carbon Sense injected two chunks: JWT setup and async preference. Skipped the Railway chunk because it doesn't exist yet. Total sent: 200 tokens."
    },
    {
        "text": "I also need a PostgreSQL database schema — a users table with hashed passwords and a login attempt tracker.",
        "say": "Fourth message. This is where it gets dramatic. The naive system is sending 1,600 tokens — most of it conversation history the model has already seen. Carbon Sense: 290 tokens. Three targeted memory chunks. The model knows the full context — JWT, async, Railway, Pydantic — because your memory layer summarized it down to 27 tokens total."
    },
    {
        ...
        "say": "Pause. Let the dashboard update. Then say:\n\nFifth message. Naive system: 2,400 tokens. Carbon Sense: 340 tokens. That is an 86% reduction. The model received the same context it needed to answer this question perfectly — compressed into targeted memory injections instead of raw history. And here's what that means in carbon terms —"
    }
]

def run_demo():
    print("Carbon Sense Demo Tester")
    print("-----------------------")
