...
async function handleStatsCommand(
  ...
  stream.markdown(`## Carbon Sense Session Stats
...
**Team projection:** If 10 developers used Carbon Sense for 50 AI queries/day, you'd avoid ~\`${annualTeam}g\` CO₂/year.

*Use \`@carbon-sense /reset\` to clear these stats.*
`);
}
...
async function handleFullRequest(
  ...
  if (!userPrompt.trim()) {
    stream.markdown('Please enter a prompt. Example: `@carbon-sense explain async/await in JavaScript`');
    return;
  }
  ...
  try {
    ...
    stream.markdown(`**Carbon Sense error:** ${msg}
