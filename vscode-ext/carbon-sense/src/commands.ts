...
export async function handleOptimizeCommand(): Promise<void> {
  ...
  if (!editor) {
    vscode.window.showWarningMessage('Carbon Sense: Open a file first.');
    return;
  }

  const selected = editor.document.getText(editor.selection);
  if (!selected.trim()) {
    vscode.window.showWarningMessage('Carbon Sense: Select a prompt to optimize.');
    return;
  }
  ...
  await vscode.window.withProgress(
    {
      ...
      title: 'Carbon Sense: Optimizing prompt...',
      cancellable: false,
    },
    async () => {
      try {
        ...
        vscode.window.showErrorMessage(
          `Carbon Sense: ${msg}. Is the backend running?`
        );
      }
    }
  );
...
export async function handleOptimizeForCopilot(): Promise<void> {
  ...
  const input = await vscode.window.showInputBox({
    title: 'Carbon Sense: Optimize Prompt for Copilot',
  ...
  await vscode.window.withProgress(
    {
      location: vscode.ProgressLocation.Notification,
      title: 'Carbon Sense: Optimizing for Copilot...',
      cancellable: false,
    },
    async () => {
      try {
        ...
        const action = await vscode.window.showQuickPick(
          ...
          {
            title: 'Carbon Sense: Optimized prompt ready',
        ...
        if (action === 'Copy optimized prompt') {
          ...
          vscode.window.showInformationMessage('Carbon Sense: Optimized prompt copied to clipboard.');
          return;
        }

        if (action === 'Replace selected text') {
          if (!editor || editor.selection.isEmpty) {
            vscode.window.showWarningMessage('Carbon Sense: No active selection to replace.');
            return;
          }
          await editor.edit((builder) => builder.replace(editor.selection, optimized));
          vscode.window.showInformationMessage('Carbon Sense: Replaced selected text with optimized prompt.');
          return;
        }
        ...
        vscode.window.showInformationMessage(
          'Carbon Sense: Opened Copilot Chat. Optimized prompt has been copied to clipboard.'
        );
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
        vscode.window.showErrorMessage(
...
export function handleShowDashboard(): void {
  const config = vscode.workspace.getConfiguration('carbon-sense');
...
export async function handleResetSession(): Promise<void> {
  ...
  resetSession();
  void refreshStatusBar();
  vscode.window.showInformationMessage('Carbon Sense: Session metrics reset.');
}
...
export async function handleSetMemorySessionId(): Promise<void> {
  ...
  const input = await vscode.window.showInputBox({
    title: 'Carbon Sense: Set Memory Session ID',
    prompt: 'Set a custom session id for @carbon-sense memory. Leave blank to auto-generate per workspace.',
    value: current,
    ignoreFocusOut: true,
  });
  ...
  if (input.trim()) {
    vscode.window.showInformationMessage(`Carbon Sense: Memory session id set to "${input.trim()}".`);
  } else {
    vscode.window.showInformationMessage('Carbon Sense: Memory session id cleared (auto project session will be used).');
  }
}
...
export async function handleClearMemorySessionId(): Promise<void> {
  ...
  vscode.window.showInformationMessage('Carbon Sense: Memory session id cleared.');
}
